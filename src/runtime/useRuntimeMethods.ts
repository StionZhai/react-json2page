...
import { ActionTypes, IUseRuntimeProps, PageStateData } from './useRuntime';
...
export interface RuntimeMethods {
  ...
  packDataFromState: (dataDefine: Record<string, any>, stateContext?: Partial<PageStateData>, pageId?: string, nodeId?: string) => Record<string, any>;
...
export function useRuntimeMethods(state, dispatch, {
  ...
  const dispatchEvent: RuntimeMethods['dispatchEvent'] = (eventName, event, nodeDefine, pageId) => {
    ...
    eventListeners.forEach(async (listener) => {
      try {
        ...
        const params = packDataFromState(actionParams, { eventDetail: event.detail }, pageId, nodeDefine.id);
  ...
  const packDataFromState: RuntimeMethods['packDataFromState'] = (dataDefine = {}, extendStateContext, pageId, nodeId) => {
    const stateContext: PageStateData = Object.assign({}, packStateContextForPage(pageId), extendStateContext);
    ...
    Object.keys(dataDefine).forEach((dataKey) => {
      ...
      if (!isLinked
        && isPlainObject(value)
        && !isEmpty(value)) {
        result[propKey] = packDataFromState(value, stateContext, pageId, nodeId);
        return;
      }
    ...
    return typeof mapNodeProps === 'function'
    ? mapNodeProps({
      nodeId,
      props: result,
      stateContext,
